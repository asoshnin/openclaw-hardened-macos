...
* `network: "none"`  
* `workspaceAccess: "rw"` (Mapped to `~/.openclaw/staging/`) — **Baseline Exception**: Agent 1 requires write access to draft files. This is a sanctioned deviation from BASELINE §3 (`"ro"` mandate), restricted exclusively to the isolated staging path. Production agents MUST use `"ro"`.  
* `capDrop: ["ALL"]`  
* `readOnlyRoot: true`  
...
* **Path A (Mobile/Remote) — ⚠️ PLANNED FUTURE FEATURE (Not Yet Implemented):** Admin replies to the Matrix bot with `/deploy <hash>`. The OpenClaw Gateway would intercept this, trigger a Deployer skill, verify the hash, ephemerally load the `.env` PAT, push the commit, and delete the staging folder. *Implementation requires a dedicated OpenClaw skill (`/deploy` handler). Currently, Path B is the only functional deployment path.*  
...
REPO_URL="github.com/asoshnin/openclaw-hardened-macos.git"  # F-030: plain URL, no markdown link syntax
BRANCH_NAME="main"
...
# Step A: Verify that the user-supplied hash matches the Red Team certificate
...
# Step B (F-031): Re-compute SHA-256 of actual staging files at deploy time.
# This closes the TOCTOU window: if staging content was modified AFTER Red Team
# approval but BEFORE this script runs, the hash mismatch will catch it.
...
if [ "$CURRENT_HASH" != "$APPROVED_HASH" ]; then
    echo "❌ TOCTOU ALERT: Staging content has changed since Red Team approval."
    echo "Expected (approved): $APPROVED_HASH"
    echo "Actual (current):    $CURRENT_HASH"
    echo "Deployment aborted to prevent deploying tampered content."
    exit 1
fi
echo "✅ Hash verified (certificate match + staging re-hash). Red Team approval confirmed."
